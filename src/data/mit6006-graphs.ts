...
Worth noticing as a warm-up special case: on an **undirected** graph, any single negative-weight edge $(u,v)$ is itself already a negative-weight cycle — walk $u \\to v \\to u$ (the same edge counted as traversed in "both directions," since undirected edges are bidirectional) for total weight $2w(u,v) < 0$. So negative-cycle detection on undirected graphs is trivial ($O(|E|)$, just scan for any negative edge) — genuinely negative-weight-cycle algorithms (Bellman-Ford) only matter for **directed** graphs, which is why the rest of this unit restricts attention to them.`,
    pitfall:
